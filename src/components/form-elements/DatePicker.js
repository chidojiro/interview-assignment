...
const DatePicker = ({orbitLib, lang="", iconPath, onChange, name, defaultValue, required, ...props}) => {
  ...
  const fieldProps = {
    ...
    "data-rs-datepicker-default-date": "",
    "data-rs-datepicker-alt-format": "Y-m-d",
    "data-rs-datepicker-date-format": "Y-m-d",
    "data-rs-datepicker-default-date": defaultValue,
    name,
    ...props,
  };
  ...
  useEffect(()=> {
    ...
    new orbitLib.DatePicker(ref.current)
    ...
    window.addEventListener('onFlatDatePickerChange', () => {
      ...
      // Target name should be same as data-rs-datepicker-input input name to allow on change event.
      target.name = name;
      onChange({
        target: target
      })
  ...
  return (
    <div ref={ref} data-rs-datepicker="">
      ...
      <input
        ...
        tabIndex="-1"
        className="flatpickr"
        data-rs-datepicker-language={lang}
        {...fieldProps}
      />
